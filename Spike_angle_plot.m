function Spike_angle_plot(path1, filename1, path2, filename2, ang_start, ref_time, n_sect, turn_b, sp_width, sp_height)
...
if nargin < 10 || isempty (ang_start) || isempty (ref_time)|| isempty(n_sect) || isempty (turn_b) || isempty (sp_width) || isempty (sp_height)
    prompt = {'Angle between horizntal plane and neraest sectors', 'Reference time', 'Number of sectors', 'Rotate borders 90 deg (0/1-no/yes)', 'Width of spike marker in seconds', 'Height of spike marker in degrees'};    
    default_data = {'8.36', '83.917', '40', '0', '6', '4'};
    options.Resize='on';
    dlg_data = inputdlg(prompt, 'Parameters', 1, default_data, options);
    
    ang_start = str2double(dlg_data{1});
    ref_time = str2double(dlg_data{2});
    n_sect = str2double(dlg_data{3});
    turn_b = str2double(dlg_data{4});
    sp_width = str2double(dlg_data{5});
    sp_height = str2double(dlg_data{6});
end
